...
- [x] `CLI-SPEC.md` 의 전체 명령
- [x] 색상 출력 + `--no-color` + `NO_COLOR`
- [x] `displayWidth()` 기반 표 정렬
- [x] `--json` 출력 모드
- [x] 사람 말로 쓴 오류 메시지 (`CLI-SPEC.md` 참고)
- [x] `state/.lock` 파일 락 (D-007)
- [x] `sched prune` 아카이브 (D-005)
- [x] 이름으로 작업자 지정 시 모호성 처리
...
- **참조를 끊는 삭제는 거부합니다.** 작업집합을 지우려는데 그것을 쓰는 작업이나 분류가 남아
  있으면 무엇을 먼저 정리해야 하는지 명령까지 적어 알려주고 멈춥니다. CLI 만으로 운영하는데
  CLI 가 깨진 데이터를 만들 수 있으면 안 됩니다.
- **겹치는 시간대는 저장 전에 막습니다.** 저장하고 나서 `validate` 가 잡는 것보다, 만들 때
  막는 편이 낫습니다. "지금 어느 시간대인가" 에 답이 둘이 되는 상태를 파일에 남기지 않습니다.
- **락은 데이터를 고치는 명령만 잡습니다.** 두 창에서 동시에 들여다보는 것은 막을 이유가
  없습니다. 다만 `today`/`now`/`show` 도 그날 스냅샷이 없으면 계산해서 저장하므로 포함됩니다.
- **`worker add` 는 ID 를 자동으로 붙입니다.** 이름이 한글이라 ID 로 쓸 수 없기 때문입니다
  (`w_1`, `w_2`, …). `--id` 로 직접 줄 수 있습니다.
- **`--json` 은 색을 끕니다.** 파이프로 받는 쪽에 이스케이프가 섞이면 안 됩니다.
- **완료 기준 1번의 범위** — 빈 폴더에서 데이터 파일 자체를 만드는 `init` 은 없습니다.
  배포본에 샘플 데이터가 들어가므로(Phase 6·7) 사용자는 파일이 있는 상태에서 시작합니다.
  그 뒤의 운영은 전부 CLI 로 됩니다.
...
- [x] `README.md` 전면 작성 (`README-PLAN.md` 의 구조와 톤 규칙 준수)
- [x] 스크린샷 또는 실제 출력 예시
- [x] `data-sample/` 을 최종 스키마에 맞춰 갱신
- [x] 릴리스 zip 에 샘플 데이터 포함 (`release.yml` 이 `data-sample/*.json` 을 `data/` 로 넣습니다)
...
- **README 의 모든 화면은 실제로 실행해 복사한 것입니다.** `README-PLAN.md` 가 "지어내면 반드시
  어긋난다" 고 못박아 두었기에, 문서의 코드블록에서 명령을 뽑아 실제로 돌리고 출력이 같은지
  대조하는 스크립트로 확인했습니다. 처음에 5건이 어긋났고 전부 실제 출력으로 고쳤습니다.
- **"처음 설정하기" 절은 빈 설정에서 이어서 실행한 결과입니다.** 작업자 3명을 등록하는 것으로
  바꿨습니다 — 1명만 등록하면 마지막 `validate` 가 인원 부족 경고를 내는데, 처음 따라 하는
  사람에게는 실패처럼 보입니다.
- **`update` 절은 화면을 지어내지 않았습니다.** 아직 릴리스가 없어 "새 버전이 있습니다" 화면을
  만들 수 없습니다. 낼 수 있는 화면(자동 업데이트가 꺼진 경우)만 싣고 나머지는 글로 적었습니다.
  첫 릴리스가 나오면 실제 화면으로 바꾸세요.
- 문서를 쓰면서 오류·경고 메시지의 조사 오류 3건을 발견해 고쳤습니다
  (`"아침청소" 의` → `"아침청소"의`, `(청소기, 밀대) 은` → `(청소기, 밀대)는`,
  `"김철수" 이 2명` → `"김철수"가 2명`).

**남은 검증** — 완료 기준 1번은 "이 프로젝트를 모르는 사람" 이 직접 해봐야 확인됩니다.
막히는 지점이 그 문서의 결함입니다.
